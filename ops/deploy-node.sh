...
NODE=""; RESTART=0; WAIT=0; RENDER=1; DRY=0; HISTORY=initial; TOOLING_ONLY=0
while [ $# -gt 0 ]; do
  case "$1" in
...
[ -n "$SEED_IN_CFG" ] || die "rendered config has no seed"
...
echo "installed \$(stat -c '%U:%G %a' $N_XAHAUD_CFG) $N_XAHAUD_CFG"
REMOTE
